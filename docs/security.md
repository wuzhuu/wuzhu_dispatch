...
### Client API Token Scope（预留字段）

> **⚠️ MVP 限制**：当前 `client_api_token.scope` 字段**尚未参与细粒度权限判断**。
> token 的实际权限由所属用户的 role（`viewer`/`operator`/`admin`/`owner`）决定。
> `scope` 为预留字段，供未来版本实现最小权限隔离。
> **不要依赖 `scope` 字段进行权限隔离。**
