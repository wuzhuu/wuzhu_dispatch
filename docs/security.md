...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `allowed_templates` | `list[str]` | `[]` | 允许使用的模板 ID，空=全部允许 |
| `allowed_modes` | `list[str]` | `["template"]` | 允许的执行模式 |
| `denied_modes` | `list[str]` | `["shell", "hermes"]` | 禁止的执行模式 |
| `allowed_target_tags` | `list[str]` | `[]` | 允许指定的 target tag，空=全部允许 |
| `allowed_node_ids` | `list[str]` | `[]` | 允许指定的 node_id |
...
| `allow_internal_network` | `bool` | `false` | 是否允许内网地址 |

如果 scope 列表为空或未设置，token 继承所属用户的 role 权限（传统模式）。
