"""Generate daily report markdown for a specific target date (cron recovery helper).
Same format as run_daily._write_daily_report / generate_daily_report_md.py,
but accepts target_date argument (defaults to previous trading day via get_target_date.py).
Owner: hermes (~/stock/scripts/). stackAnalys/ untouched.
"""
...
sys.path.insert(0, str(PROJECT_ROOT / "stackAnalys"))
...
def get_target() -> str:
    r = subprocess.run(
        [str(PROJECT_ROOT / "stackAnalys/.venv/bin/python"),
         str(PROJECT_ROOT / "scripts/get_target_date.py")],
